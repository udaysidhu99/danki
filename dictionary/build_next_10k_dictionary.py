#!/usr/bin/env python3
"""
Build the NEXT 10,000-word German-English dictionary (words 10,001-20,000) using OpenAI API.
...
# Next 10K: words 10,001 - 20,000 from de_50k.txt
WORD_START_IDX = 10000
WORD_COUNT = 10000
FILE_PREFIX = "german_english_dict_next_10k"
...
def load_checkpoint(worker_id):
    ...
    checkpoint_file = f"checkpoint_next_10k_worker_{worker_id}.json"
...
def build_worker_chunk(worker_id, words, api_key, output_file):
    ...
    log_file = f"{FILE_PREFIX}_worker_{worker_id}.log"
    ...
    with open(log_file, 'w', encoding='utf-8') as log:
        log.write("=" * 60 + "\n")
        log.write(f"German-English Dictionary Builder (Next 10K) - Worker {worker_id}\n")
        log.write("=" * 60 + "\n")
        log.write(f"Word range: {WORD_START_IDX + 1} - {WORD_START_IDX + WORD_COUNT} from de_50k.txt\n")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python build_next_10k_dictionary.py <worker_id> <api_key>")
        sys.exit(1)
    
    worker_id = int(sys.argv[1])
    api_key = sys.argv[2]
    
    # Load words 10,001 - 20,000 from de_50k.txt
    word_list_path = "de_50k.txt"
    all_words = load_word_list(word_list_path, start_idx=WORD_START_IDX, count=WORD_COUNT)
    
    print(f"Loaded {len(all_words)} words (positions {WORD_START_IDX + 1}-{WORD_START_IDX + len(all_words)} in de_50k.txt)")
    
    # Number of workers is passed via environment or defaults to 8
    ...
    output_file = f"{FILE_PREFIX}_part_{worker_id}.json"
